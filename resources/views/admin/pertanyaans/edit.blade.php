...
@section('subtitle', 'Perbarui pertanyaan kuisioner untuk jurusan tertentu')

@section('content')
    <div class="rounded-3xl bg-white p-6 border border-slate-200 shadow-sm">
        <form action="{{ route('admin.pertanyaans.update', $pertanyaan) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="kriteria_id" class="mt-2 w-full rounded-2xl border border-slate-300 bg-slate-50 px-4 py-3">
                        ...
                        @foreach($kriterias as $kriteria)
                            <option value="{{ $kriteria->id }}" {{ old('kriteria_id', $pertanyaan->kriteria_id) == $kriteria->id ? 'selected' : '' }}>{{ $kriteria->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
